...
app.get('/api/debug/admin-status', async (req, res) => {
  try {
    ...
    let adminQuery, countQuery;
    if (isProduction) {
      // PostgreSQL syntax
      adminQuery = `SELECT id, phone, role, status FROM users WHERE role = $1`;
      countQuery = `SELECT COUNT(*) as count FROM users`;
    } else {
      // SQLite syntax
      adminQuery = `SELECT id, phone, role, status FROM users WHERE role = ?`;
      countQuery = `SELECT COUNT(*) as count FROM users`;
    }

    const result = await query(adminQuery, ['admin']);
    ...
    const totalUsers = isProduction ? countResult.rows[0].count : countResult.rows[0].count;
